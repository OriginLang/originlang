...
OriginLang 面向插件开发者的发行边界。

Developer Kit 让第三方开发者无需取得完整宿主源码、Electron 应用或内置 JRE/Python 运行时，也能创建、构建、测试与分发插件。它由多个可独立版本化的组件构成，而不是一个要求所有语言共用的构建系统。
...
最终宿主决定如何满足该声明：Electron 可使用其内置 Node 承载 Node 插件；桌面发行物可携带受控 JRE 或 CPython；服务端与 CLI 宿主可使用管理员配置的运行时。运行时适配与解析规则归属 `adapters/`，详见 `../adapters/README.md`。
...
`origin` 不是可用的默认命令名：已有 Cursor Origin CLI 和其他项目占用该可执行命令。当前保留 `ol` 作为暂定名称；若需要可读性更高的替代名，应以完整名称 `originlang` 等候选项进行单独的名称、包注册与商标审查。
